...
public class CookieValuesRepositoryJdbcImpl implements CookieValuesRepository {
    ...
    private RowMapper<CookieValue> cookieRowMapper = row -> CookieValue.builder()
            ...
            .user(usersRepository.findById(row.getLong("student_id")))
            .build();
    ...
    public boolean cookieExist(String cookie) {
        if (template.SelectQuery(SQL_COOKIE_EXIST, cookieRowMapper, cookie).size() > 0) {
            return true;
        } else {
            return false;
        }
    ...
    @Override
    public CookieValue findById(Long id) {
        return null;
    }
